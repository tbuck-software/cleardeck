...
Der Workflow `windows-recovery.yml` prüft die direkte Installation über 2.1.0 und 2.2.0, das Update aus 2.2.1 sowie ein anschließendes Update aus 2.3.0 auf einen nur für den Test gebauten Nachfolger. Branchläufe veröffentlichen nichts. Erst der Tag `v2.3.0` führt nach erneuter erfolgreicher Prüfung zur Veröffentlichung der Windows-Dateien; zuvor werden die hochgeladenen Dateien erneut heruntergeladen und geprüft. Der frühere Mac-Feed wird nicht wiederverwendet; die Windows-Reparatur liefert kein neues Mac-Paket aus.
