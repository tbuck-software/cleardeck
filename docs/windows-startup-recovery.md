...
- `assets/app-update.yml` verweist auf private GitHub-Releases. Aktuelle Builds lesen `GH_TOKEN` ausschließlich zur Laufzeit im Hauptprozess; ohne Token meldet die App den privaten Feed als manuellen Updatefall. Der Zugriff des untersuchten alten Clients auf die Release-Metadaten belegt, dass dort die Berechtigungen ausreichten.
