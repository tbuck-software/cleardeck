...
describe('Deckelung bei Minderjährigen (JArbSchG § 29 Abs. 2)', () => {
  it('verkürzt auf ein halbes Jahr', () => {
    ...
    expect(effectiveIntervalMonths(12, '2010-01-01', '2026-09-05')).toBe(MINOR_MAX_INTERVAL_MONTHS);
    expect(nextDueDate(12, '2010-01-01', '2026-09-05')).toBe('2027-03-05');
  });

  it('verlängert ein ohnehin kürzeres Intervall nicht', () => {
    expect(effectiveIntervalMonths(3, '2010-01-01', '2026-09-05')).toBe(3);
  });

  it('richtet sich nach dem Alter bei der Durchführung', () => {
    // Am 05.09.2026 bereits 18 — ab da gilt das reguläre Intervall.
    expect(effectiveIntervalMonths(12, '2008-08-01', '2026-09-05')).toBe(12);
    // Einen Monat vorher noch 17 — dann greift die Deckelung.
    expect(effectiveIntervalMonths(12, '2008-10-01', '2026-09-05')).toBe(6);
  });

  it('lässt einer minderjährigen Person kein Zwei-Jahres-Thema durchgehen', () => {
    // Der Fall, den eine Prüfung am Fälligkeitstag übersehen hätte.
    expect(effectiveIntervalMonths(24, '2010-01-01', '2026-09-05')).toBe(6);
  });

  it('greift ohne Geburtsdatum nicht, statt ein Alter zu raten', () => {
    expect(effectiveIntervalMonths(12, null, '2026-09-05')).toBe(12);
    expect(effectiveIntervalMonths(12, '0000-05-20', '2026-09-05')).toBe(12);
  });
});
