...
const instructionState = (instruction: EmployeeInstruction, today: string) => {
  if (instruction.completedAt) return { label: 'erledigt', tagClass: 'tag-accent-2' };
  ...
  if (daysBetween(today, instruction.dueDate) <= 30) return { label: 'bald fällig', tagClass: 'tag-accent' };
...
const EmployeeDetail = ({
  ...
  const openCompetencies = competencies.filter((entry) => !entry.level).length;
  ...
  return (
    <div className="cd-page cd-detail">
      <header className="cd-detail-header">
        ...
        <div style={{ flex: 1, minWidth: 260 }}>
          ...
          <p style={{ margin: '10px 0 0', fontSize: 14 }}>
            {employee.note || 'Keine Notiz — über „Bearbeiten“ ergänzen.'}
          </p>
        </div>
      ...
      <div className="cd-facts">
        ...
        <div>
          <div className="cd-fact-value">{fte2(employee.fte)}</div>
          <div className="cd-fact-label">VZÄ (Basis {baseHours} h)</div>
        </div>
        <div>
          <div className="cd-fact-value">{employee.birthDate ? formatDateDE(employee.birthDate) : 'fehlt'}</div>
      ...
      {tab === 'comp' && (
        <section>
          <div className="cd-section-head">
            <div>
              ...
              <p className="cd-muted-14" style={{ margin: '4px 0 0' }}>
                Stufe 1 unterwiesen · 3 unter Aufsicht · 5 kann anleiten. Klick auf eine Zeile öffnet die Freigabe.
              </p>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              {suggestedCompetencyCount > 0 && (
                <button type="button" className="btn btn-secondary" onClick={onOpenSuggestedCompetencies}>
          ...
          <div className="cd-panel">
            {competencies.length === 0 && <div className="cd-empty">Noch keine Kompetenzen zugeordnet.</div>}
            {competencies.map((competency) => {
              const level = competency.level ?? 0;
              const tagClass = !level ? 'tag-neutral' : level >= 4 ? 'tag-accent-2' : 'tag-accent';
              return (
                <button
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className="cd-muted-13">
                      ...
                      {competency.approvedAt ? `freigegeben ${formatDateDE(competency.approvedAt)}` : 'keine Freigabe'}
                    </div>
                  </div>
                  <div className="cd-level-dots" aria-label={`Stufe ${level} von 5`}>
                    {[1, 2, 3, 4, 5].map((step) => (
                      <span
                        key={step}
                        style={{
                  ...
                  <span className={`tag ${tagClass}`} style={{ flex: 'none', minWidth: 110, justifyContent: 'center' }}>
                    {level ? `${level} · ${COMPETENCY_LEVELS[level]}` : 'Offen'}
                  </span>
                </button>
              );
            })}
      ...
      {tab === 'instr' && (
        <section>
          <div className="cd-section-head">
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              {availableInstructionCount === 0 && (
                <span className="cd-muted-13">Zu jeder Einweisung läuft bereits ein offener Eintrag.</span>
              )}
              <button
                type="button"
                className="btn btn-primary"
          ...
          <div className="cd-panel">
            {instructions.length === 0 && <div className="cd-empty">Noch keine Einweisungen zugeordnet.</div>}
            {instructions.map((instruction) => {
              const state = instructionState(instruction, today);
              return (
                <button
                ...
                >
                  ...
                  <span className="cd-muted-13" style={{ flex: 'none' }}>
                    {instruction.dueDate ? `fällig ${formatDateDE(instruction.dueDate)}` : 'ohne Frist'}
                  </span>
                  <span className={`tag ${state.tagClass}`} style={{ flex: 'none', minWidth: 90, justifyContent: 'center' }}>
                    {state.label}
                  </span>
                </button>
              );
              ...
              return (
                ...
                >
                  ...
                  <div className="cd-timeline-rail">
                    <span
                      style={{
                        background: isPeriod ? 'var(--color-accent-2-500)' : 'var(--color-accent-500)',
                      }}
                    />
                    <span />
                  </div>
